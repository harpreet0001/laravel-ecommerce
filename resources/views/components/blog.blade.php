	...
	<div class="container">
		<div class="megatan-blog_content">
                      ...
                      <div class="owl-item active" style="width: 406.664px; margin-right: 15px;">
						<div class="item">
						    <div class="megatan-blog-card">
							    <figure>
								    <span class="p-date p-date-image">{!! blogDateFormat($latestBlog->created_at) !!}</span>
								    <a href="{{route('blog-detail',$latestBlog->slug)}}">
									   <img src="{{imagePath($latestBlog->image)}}" class="img-fluid">
								    </a>
								...
								<figcaption>
									<h2 class="text-center">
										<a href="javascript:void(0);">{{$latestBlog->title}}</a>
									</h2>
									<p>{!! substr($latestBlog->content,0,50) !!}</p>
									<a href="{{route('blog-detail',$latestBlog->slug)}}" class="btn read-more-btn">Read more<i class="fas fa-arrow-right"></i></a>
								</figcaption>
						</div>		
					  </div>
				    </div>
			...
			<!-- carousel end-->
				...
				<div class="owl-stage-outer">
					<div class="owl-stage" style="transform: translate3d(0px, 0px, 0px); transition: all 0s ease 0s; width: 1265px;">
						@foreach($mostReadableBlogs as $mostReadableBlog)
							<div class="owl-item active" style="width: 406.664px; margin-right: 15px;">
								<div class="item">
						        	<div class="megatan-blog-card">
									   <figure>
											<span class="p-date p-date-image">{!! blogDateFormat($mostReadableBlog->created_at) !!}</span>
											<a href="{{route('blog-detail',$mostReadableBlog->slug)}}">
												<img src="{{imagePath($mostReadableBlog->image)}}" class="img-fluid">
											</a>
											<div class="post-stats">
												<span class="p-author">Admin</span>
										...
										<figcaption>
											<h2 class="text-center">
												<a href="javascript:void(0);">{{$mostReadableBlog->title}}</a>
											</h2>
											<p>{!! substr($mostReadableBlog->content,0,50) !!}</p>
											<a href="{{route('blog-detail',$mostReadableBlog->slug)}}" class="btn read-more-btn">Read more<i class="fas fa-arrow-right"></i></a>
										</figcaption>
							        </div>		
						        </div>
					        </div>
